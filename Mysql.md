...
>>* 启动服务：sudo /etc/init.d/mysql start | stop | restart重启  或者 service mysql   start|stop|restart
...
CREATE [OR REPLACE] 替换原有视图VIEW [view_name]视图名字 AS [SELECT_STATEMENT];11 rows in set (0.00 sec)

‘create view class_1 as select name,age,score from cls1 where score>80;’

mysql> select p.name,d.dname from person as p,dept as d where d.id=p.dept_id;
ERROR 1054 (42S22): Unknown column 'p.dept_id' in 'where clause'
...
* 作用
  ...
  2. 简化高频复杂操作的过程，就像一种**对复杂操作的封装**。
  3. 提高**安全性**，可以给不同用户提供不同的视图
  4. 让数据更加清晰
* 缺点
...
　　create procedure 存储过程名(形参列表) 一定没有返回值
...
- 第三范式： 在**第二范式的基础上属性不传递依赖**，多个表不会出现相同字段，即每个属性于其它非主属性。要求一个关系中不包含已在其它关系已包含的非主关键字信息。
...
-  尽量选择数据类型**占空间少**，在where ，group by，order by中**出现的频率高**的字段建立索引
- explain 放在查询语句前面可以获取查询计划，建立合适索引
- 尽量**避免使用 select *** ...;用具体字段代替 * ,**不要返回用不到的任何字段** 
- 少使用like %查询，否则会全表扫描
- 子查询优化为join查询
- 控制使用自定义函数
...
- 尽量避免 or 连接条件,否则放弃索引全表扫描，可以用**union**代替
...
水平拆分 ： 减少每个表的数据量，通过hash key（哈西key）进行划分然后拆成多个表
...
1、表能根据实际需求复制数据
2、复制表时**不会把KEY属性**复制过来

**语法**

```mysql
create table 表名 select 查询命令;create table student select * from cls1 where score>90;
```
...
4.vi mysqld.cnf #找到44行左右,加 # 注释  i调用输入模式
   #bind-address = 127.0.0.1
   [mysqld]
   character_set_server = utf8
5.保存退出 :wq
6.service mysql restart  重启mysql服务
7.修改用户表host值 
  use mysql;
...
1. 用root用户登录mysql
   mysql -uroot -p123456
2. 添加用户 % 表示自动选择可用IP
   CREATE USER 'username'@'host' IDENTIFIED BY 'password';  host 可以替换成%
3. 授权 *表达所有库，*表达所有表
